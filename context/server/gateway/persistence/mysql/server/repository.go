...
// 新規作成します
func (r *Repository) Create(u *server.Server) error {
	qs := fmt.Sprintf(
...
// 更新します
func (r *Repository) Update(u *server.Server) error {
	qs := fmt.Sprintf(
...
func (r *Repository) FindByID(id model.ID) (*server.Server, error) {
	...
	for rows.Next() {
		...
		if err = rows.Scan(&ro.ID, &ro.Content, &ro.Created, &ro.Updated); err != nil {
			return nil, errors.NewError("サーバーが見つかりません", err)
		}
		res = append(res, ro)
	}
	...
	if len(res) > 1 {
		return nil, errors.NewError("サーバーが複数見つかりました")
	}
...
//
// `ID:構造体`のmapを返します。
func (r *Repository) FindAll() (map[string]*server.Server, error) {
	res := map[string]*server.Server{}
